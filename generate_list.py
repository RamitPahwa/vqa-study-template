...
def sep_sessions(df,seed):
    ...
    first_session_contents = random.Random(seed).sample(unique_contents,int(len(unique_contents)//3))
    print("========")
    temp = [i for i in unique_contents if i not in first_session_contents]
    second_session_contents = random.Random(seed).sample(temp, int(len(temp)//2))
    third_session_contents = [i for i in temp if i not in second_session_contents]
    print(bool(set(first_session_contents) & set(second_session_contents)))
    print(bool(set(second_session_contents) & set(third_session_contents)))
    print(bool(set(third_session_contents) & set(second_session_contents)))
    first_session_df = df[df['content'].isin(first_session_contents)]
    second_session_df = df[df['content'].isin(second_session_contents)]
    third_session_df = df[df['content'].isin(third_session_contents)]
    return first_session_df,second_session_df, third_session_df

def get_lists(df):
    for seed in range(1,80):
        first_session_df,second_session_df, third_session_df = sep_sessions(df,seed)
